...
selremove_command:
    ...
    debug: false
    name: selremove
    permission: selector_tool.selremove
    description: Removes relevant flag (for clean-up purposes)
    usage: /selremove
    script:
    - if !<player.inventory.contains_item[selector_tool_item]>:
        - flag <player> seltool_selection:!
        - flag <player> seltool_type:!
        - narrate "<&e>Seltool flags successfully cleared!"
    - else:
        - narrate "<&c>You may not have the selector tool in your inventory when you clean up the flags!"
